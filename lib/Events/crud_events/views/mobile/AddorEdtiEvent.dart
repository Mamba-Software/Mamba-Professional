...
import 'package:mamba/Events/crud_events/widgets/responsive_addeditevent.dart';
...
class _AddOrEditEventState extends State<AddOrEditEvent>
  ...
  Widget eventScreen(CrudEventLoaded state) {
  ...
  Widget widgetWeb(CrudEventLoaded state) {
    return Scaffold(
      appBar: AppBar(
                    ...
                    if (!state.oldEvent.joinedMembersList!
                      ...
                      if (!state.oldEvent.isRecurrent!) {
                        ...
                        var result = await showDialog(
                            ...
                            builder: (_) {
                              return DeleteConfirmationDialog(
                                  text: context.l10n.deleteEventConfirmation);
                            });
                        if (result) {
                          context.read<CrudEventCubit>().deleteEventFunction(
                              context, state.oldEvent, state.isPrivate);
                          // Pop to Last Page
                          Navigator.pop(context, false);
                        }
                      } else {
                      ...
                      var result = await showDialog(
                          ...
                          builder: (_) {
                            return DeleteConfirmationDialog(
                                text: context.l10n.deleteClientsWithPurchases,
                                permitDelete: false);
                          });
                    }
                  },
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: FloatingActionButton.extended(
                ...
                shape: const StadiumBorder(),
                onPressed: () async {
                  if (_selectedIndex == 0 && state.isValidated[0] == true) {
                    if (!state.isNew) {
                      mixpanel!.track('edit_event_datetime',
                          properties: {'isPrivate': state.isPrivate});
                    } else {
                    ...
                    if (state.isValidated.every((bool value) => value)) {
                      String eventTimeTime = StringUtils().hourMinutesToString(
                          state.newEvent.startDate!.hour,
                          state.newEvent.startDate!.minute);
                      if (state.isNew) {
                        ...
                        if (!state.newEvent.isRecurrent!) {
                          context.read<CrudEventCubit>().updateEventFunction(
                              context,
                              state.newEvent,
                              state.oldEvent,
                              _setNotificationBefore(
                ...
                backgroundColor: _selectedIndex == 2 &&
                        state.isValidated[2] == true
                    ? Colors.green
                    : _selectedIndex == 0 && state.isValidated[0] == true
                        ? Theme.of(context).colorScheme.secondary
                        : _selectedIndex == 1 && state.isValidated[1] == true
                            ? Theme.of(context).colorScheme.secondary
                            : Colors.grey,
                icon: Container(),
                label: state.isNew
