...
class AgeRangeState extends State<AgeRange> {
  ...
  void mountStat() {
    ...
    for (int i = 0; i < filteredUsers.length; ++i) {
      age = calculateAge(
          DateFormat('dd-MM-yy').parse(filteredUsers[i].dateOfBirth!));
      if (age < 12) {
  ...
  void addDayInTotalEvent(String range, int number) {
    ...
    clientsAgeTotal = ClientsAge(range, clientsAge[number]);
...
*/
                                ...
                                xValueMapper: (ClientsAge events, _) =>
                                    events.range,
                                yValueMapper: (ClientsAge events, _) =>
                                    events.age,
                              ),
                            ]))),
              ),
