...
class TimeToTimeOfferState extends State<TimeToTimeOffer> {
  ...
  setHour(int initHour, int endHour, String totalHour, Event event) {
    if (int.parse(event.hour!) >= initHour &&
        int.parse(event.hour!) < endHour) {
      if (!mapHours.containsKey(totalHour)) {
        mapHours[totalHour] = event.numClients!;
      } else {
        mapHours.update(totalHour, (value) => value + event.numClients!);
      }
    }
  }
    ...
    return isLoading
        ...
        : Stack(
            ...
            children: [
              ...
              filteredEvents.isEmpty || clientsInEvents == 0
                  ? Column(
