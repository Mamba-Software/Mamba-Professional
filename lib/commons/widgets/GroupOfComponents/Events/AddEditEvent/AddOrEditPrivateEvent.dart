...
import 'package:mamba_castelldefels/commons/widgets/Components/CupertinoSelect/SelectDurationDialog.dart';
...
import 'package:mamba_castelldefels/commons/widgets/GroupOfComponents/Dialogs/ActionDialogs/DeleteRecurrentEventDialog.dart';
import 'package:mamba_castelldefels/commons/widgets/GroupOfComponents/Dialogs/ActionDialogs/EditRecurrentEventDialog.dart';
...
import 'package:mamba_castelldefels/commons/widgets/GroupOfComponents/LocationAutoComplete/MyLocationsSelect.dart';
...
class _AddOrEditPrivateEventState extends State<AddOrEditPrivateEvent>
  ...
  Future<void> getEventBonos() async {
    eventBonos = await _eventDataService.getEventBonos(
        widget.eventId!, currentBrand.id!);
                                                          ...
                                                          children: [
                                                            ...
                                                            Flexible(
                                                              child: Text(
                                                                AppLocalizations.of(
                                                                        context)!
                                                                    .bonosDescriptionPrivateWarning,
                                                                textAlign:
                                                                    TextAlign
                                                                        .left,
                                                                style: Theme.of(
                                                                ...
                                                                onPressed: () {
                                                                  ...
                                                                  setState(() {
                                                                    if (selectedBonos
                                                                        .contains(
                                                                            bono.id!)) {
                                                                      if (brandClientsSelected.any((client) =>
                                                                          client
                                                                              .purchaseId !=
                                                                          "")) {
                                                                        errorBonos =
    ...
    if (!isRecurrent) {
      ...
      setState(() {
        isRecurrentLoadingText =
            "${AppLocalizations.of(context)!.creating} ${AppLocalizations.of(context)!.events.toLowerCase()}... ($currentEvent/$currentEvent)";
      });
      ...
      if (_value == 1) {
        ...
        for (var i = 0; i < 6; i++) {
          if (values[weekDay - 1]!) {
            ...
            setState(() {
              isRecurrentLoadingText =
                  "${AppLocalizations.of(context)!.creating} ${AppLocalizations.of(context)!.events.toLowerCase()}... ($currentEvent/$totalEvents)";
            });
            currentEvent += 1;
  ...
  Future<void> _deleteEventFunction() async {
    ...
    setState(() {
      ...
      // Updating Loading Text
      isRecurrentLoadingText =
          "${AppLocalizations.of(context)!.deleting} ${AppLocalizations.of(context)!.events.toLowerCase()}... ($currentEvent/$currentEvent)";
    });
    // Delete Event Call
    ...
    mixpanel!.track('delete_event_completed',
        properties: {'isPrivate': true, 'isRecurrent': false});
    // Pop to Last Page
  ...
  Future<void> _updateEventFunction() async {
    ...
    setState(() {
      ...
      // Updating Loading Text
      isRecurrentLoadingText =
          "${AppLocalizations.of(context)!.editing} ${AppLocalizations.of(context)!.events.toLowerCase()}... ($currentEvent/$currentEvent)";
    });
    // Get Random Photo if no Image Selected
    if (isRandomImage) {
      eventImageUrl =
  ...
  Future<void> _deleteRecurrentEventFunction() async {
    ...
    for (var i = index; i < eventGroupIdsList.length; i++) {
      ...
      setState(() {
        isRecurrentLoadingText =
            "${AppLocalizations.of(context)!.deleting} ${AppLocalizations.of(context)!.events.toLowerCase()}... ($currentEvent/$totalEvents)";
      });
      currentEvent += 1;
      // Event Id
    ...
    mixpanel!.track('delete_event_completed',
        properties: {'isPrivate': true, 'isRecurrent': true});
    // Pop to Last Page
    Navigator.pop(context, false);
  }
  ...
  Future<void> _updateRecurrentEventFunction() async {
    ...
    for (var i = index; i < eventGroupIdsList.length; i++) {
      ...
      setState(() {
        isRecurrentLoadingText =
            "${AppLocalizations.of(context)!.editing} ${AppLocalizations.of(context)!.events.toLowerCase()}... ($currentEvent/$totalEvents)";
      });
      currentEvent += 1;
      // Event Id
      String eventId = eventGroupIdsList[i];
