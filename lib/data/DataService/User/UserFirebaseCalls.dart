...
class UserFirebaseCalls {
  ...
  Future<Event> getLastUserEvent(String? userId) async {
    ...
    QuerySnapshot querySnapshot = await _firestore
        ...
        .collection("Events")
        .get();
    QuerySnapshot querySnapshotPrivate = await _firestore
        .collection(users)
        .doc(userId)
        .collection("Events")
        .doc('Private Events')
    ...
    for (int i = 0; i < querySnapshot.docs.length; i++) {
      if (querySnapshot.docs[i].id != "Private Events") {
        events.add(Event.fromObjectAllData(
            querySnapshot.docs[i].id, querySnapshot.docs[i]));
      }
    }
    for (int i = 0; i < querySnapshotPrivate.docs.length; i++) {
      privateEvents.add(Event.fromObjectAllData(
          querySnapshot.docs[i].id, querySnapshot.docs[i]));
    }
    ...
    if (privateEvents.isEmpty) {
      ...
      if (events.isEmpty) {
        ...
        if (events[0]
                .doneAt!
                .toDate()
                .compareTo(privateEvents[0].doneAt!.toDate()) ==
            0) {
          return events[0];
        } else {
          return privateEvents[0];
  ...
  Future<List<Bono>> getUserActiveBonos(
  ...
  Future<int> addUser(
      ...
      [bool definePassword = false]) async {
  ...
  // Register User
  Future<bool> addUserGoogleOrApple(
      UserCredential authResult, String idioma) async {
    final DateTime now = DateTime.now();
    final DateFormat formatter = DateFormat('dd-MM-yyyy');
    final String formatted = formatter.format(now);
  ...
  Future<void> updateUserZoomScale(
      String userId, String brandId, double zoomScale) async {
    await _firestore
        .collection(brands)
        .doc(brandId)
        .collection("Users")
...
// Add User
  ...
  Future<void> updateUserPurchase(
      String userId, String brandId, Bono bono, Purchase purchase) async {
  ...
  Future<void> deleteUserBono(
      String userId, String brandId, String bonoId, String purchaseId) async {
    // Update the IsActive Field on all Collections
