...
class JoinConfirmationDialogBonos extends StatefulWidget {
  ...
  final String userId;
  const JoinConfirmationDialogBonos({Key? key, required this.text, required this.event, required this.bonos, required this.brand, required this.userId}) : super(key: key);

  @override
  _JoinConfirmationDialogBonosState createState() => _JoinConfirmationDialogBonosState();
}

class _JoinConfirmationDialogBonosState extends State<JoinConfirmationDialogBonos> {
  // Boolean IsLoading
              ...
              builder: (context, snapshot) {
                if (!snapshot.hasData) {
                  ...
                  userBonosPurchases.removeWhere((userPurchase) {
                    int index = widget.bonos.indexWhere((eventBono) => eventBono.id! == userPurchase.bonoId!);
                    if (index == -1) {
                      return true;
                    } else {
                      return false;
                  ...
                  return Column(
                      children: [
                        ...
                        userBonosPurchases[_currentPage].bono!.condition!.cancelTime != 0 ? Column(
                          children: [
                            ...
                            Padding(
                              ...
                              child: Container(
                                ...
                                child: ListTile(
                                    ...
                                    minLeadingWidth: MediaQuery.of(context).size.width * 0.07,
                                    leading: Icon(
                                    ...
                                    title: Text(
                                      AppLocalizations.of(context)!.cancelTimeAt + " " + userBonosPurchases[_currentPage].bono!.condition!.cancelTime.toString() + " " + AppLocalizations.of(context)!.hours.toLowerCase(),
                        ...
                        onPressed: () async {
                              if (widget.bonos.isEmpty) {
                                ...
                                if (userBonosPurchases.isNotEmpty) {
                                  ...
                                  bonoSelected = userBonosPurchases[_currentPage].bono!;
                      ...
                      child: ClipOval(
                        child: Material(
                          ...
                          child: InkWell(
                            ...
                            child: const Icon(Icons.event_available_outlined, color: Colors.white, size: 40,), // icon
                          ),
                        ),
                      ),
                    ),
