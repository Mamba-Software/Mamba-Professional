...
class UserPurchasesCubit extends Cubit<UserPurchasesState> {
  ...
  UserPurchasesCubit(this.userId, this.brandId) : super(const UserPurchasesInitial()) {
  ...
  Future<void> fetchInitialUserPurchases() async {
    try {
      ...
      purchasesList = await _purchaseDataService.getBrandUserPurchases(userId, brandId, startDate, endDate, true);
  ...
  /// FETCH MORE BRAND
  Future<List<PurchaseHistoryModel>> fetchMoreUserPurchases(String lastPurchaseId) async {
    try {
      print("Getting More Brand Purchases");
      // Vars
      List<PurchaseHistoryModel> purchasesHistoryListsPurchases = [];
      // Fetch more purchases with the new dates
      List<Purchase> morePurchases = await _purchaseDataService.getBrandUserPurchases(userId, brandId, startDate, lastFetchedPurchaseDate);
