...
class BrandPurchasesCubit extends Cubit<BrandPurchasesState> {
  ...
  Future<void> fetchInitialBrandPurchases() async {
    try {
      ...
      purchasesList = await _purchaseDataService.getBrandPurchases(
          brandId, startDate, endDate, true);
      ...
      for (Purchase p in purchasesList) {
        // Get User
        Usuario user = usersList.firstWhere((element) => element.id == p.userId,
            orElse: () => Usuario());
        if (user.id == null) {
  ...
  /// FETCH MORE BRAND
  Future<List<PurchaseHistoryModel>> fetchMoreBrandPurchases(
    ...
    try {
      ...
      List<Purchase> morePurchases = await _purchaseDataService
          .getBrandPurchases(brandId, startDate, lastFetchedPurchaseDate);
  ...
  Future<void> openPurchasesStream() async {
    try {
      _subscriptionPurchases =
          _brandDataService.getBrandPurchasesStream(brandId).skip(1).listen(
        (querySnapshot) async {
