...
class ScriptsDatabaseService {
    ...
    try {
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        if (user.name == "null" || user.name == null) {
          ...
          print('Adding "Notifications" subcollection');
          print('-----------------------------\n');
          QuerySnapshot querySnapshotNotif = await _firestore
          ...
          for (var i = 0; i < querySnapshotNotif.docs.length; i++) {
            String notifId = querySnapshotNotif.docs[i].id;
            print('Notification with ID : $notifId');
            DocumentSnapshot documentSnapshot = querySnapshotNotif.docs[i];
            await _firestore
                .collection(users)
                .doc(user.id!)
                .collection("Notifications")
          ...
          print('Adding "Errors" subcollection');
          print('-----------------------------\n');
          QuerySnapshot querySnapshotErrors = await _firestore
              .collection(errors)
              .where("userID", isEqualTo: user.id!)
              .get();
          for (var i = 0; i < querySnapshotErrors.docs.length; i++) {
          ...
          QuerySnapshot querySnapshotEvents;
          if (user.isTrainer!) {
            querySnapshotEvents = await _firestore
                .collection(events)
                .where("selectedTrainers", arrayContains: user.id!)
                .get();
          } else {
            querySnapshotEvents = await _firestore
                .collection(events)
                .where("joinedMembers", arrayContains: user.id!)
                .get();
          }
          for (var i = 0; i < querySnapshotEvents.docs.length; i++) {
            String eventId = querySnapshotEvents.docs[i].id;
            DocumentSnapshot documentSnapshot = querySnapshotEvents.docs[i];
            print('Event with ID : $eventId');
            int numClients = documentSnapshot.get("joinedMembers").length;
            int numTrainers = documentSnapshot.get("selectedTrainers").length;
            await _firestore
                .collection(users)
                .doc(user.id!)
                .collection("Events")
    ...
    try {
      ...
      */
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        print('Updating numTrainers and numClients document of Brand');
        await _firestore.collection(brands).doc(brandId).update({
          "numClients": documentSnapshot.get("numberClients"),
          "numTrainers": documentSnapshot.get("numberTrainers"),
        });
        print('Create Room for Brand ...');
        print('-----------------------------');
        final room = await FirebaseChatCore.instance.createGroupRoom(
            ...
            users: []);
        print(
            'Assign the new group room id to field "roomId" of Brand Document');
        await _firestore.collection(brands).doc(brandId).update({
          "roomId": room.id,
        });
        print('Add all members of Brand to this Group Room');
        ...
        print('Adding "Users" subcollection');
        print('-----------------------------\n');
        QuerySnapshot querySnapshotUsers = await _firestore
            .collection(users)
            .where("brandID", isEqualTo: brandId)
            .get();
        for (var i = 0; i < querySnapshotUsers.docs.length; i++) {
          String userId = querySnapshotUsers.docs[i].id;
          DocumentSnapshot documentSnapshot = querySnapshotUsers.docs[i];
          Usuario user = Usuario.fromObjectAllData(userId, documentSnapshot);
          print('User with ID : $userId');
          int role = 0;
          if (documentSnapshot.get("isTrainer")) {
            if (userId == brand.adminID) {
              role = 1;
            } else {
              role = 5;
        ...
        print('Adding "Locations" subcollection');
        print('-----------------------------\n');
        QuerySnapshot querySnapshotLocations = await _firestore
            .collection(locations)
            .where("brandID", isEqualTo: brandId)
            .get();
        for (var i = 0; i < querySnapshotLocations.docs.length; i++) {
          String locationId = querySnapshotLocations.docs[i].id;
          DocumentSnapshot documentSnapshot = querySnapshotLocations.docs[i];
          Location location =
              Location.fromObjectAllData(locationId, documentSnapshot);
          print('Location with ID : $locationId');
          await _firestore
              .collection(brands)
              .doc(brandId)
  ...
  Future<bool> migrateEventDataFebruary6th() async {
    try {
      ...
      QuerySnapshot querySnapshot = await _firestore
          .collection(events)
          .where("brandID", isEqualTo: "ef80f103-824c-4f4b-9764-8fe4863c8c4f")
  ...
  Future<bool> migrateLocationDataFebruary6th() async {
    try {
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        QuerySnapshot querySnapshotEvents = await _firestore
            .collection(events)
            .where("brandID", isEqualTo: location.brandID)
  ...
  Future<bool> migrateResizeCompressUserImages() async {
    try {
      ...
      // TEST IN DEVELOPMENT
      QuerySnapshot querySnapshot =
          await _firestore.collection("7777 Users").get();
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        DocumentSnapshot documentSnapshot = querySnapshot.docs[i];
        Usuario user =
            Usuario.fromObjectAllData(documentSnapshot.id, documentSnapshot);
        ...
        if (user.name == "null" || user.name == null) {
          ...
          if (noImageUrl == user.imageUrl) {
            ...
            var compressedSize = await ImageUtils()
            ...
            var result = await _userDataService.updateUserPhoto(
                user.id!, File(compressedFileImage.path));
            print("Image succesfully uploaded!");
            print("\n");
            List<Brand> brandsList =
                await _brandDataService.getAllBrandsFromUser(user.id!);
  ...
  Future<bool> migrateResizeCompressBrandImages() async {
    try {
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        // Upload Photo de Firebase and Update
        print("Uploading image ...");
        await _brandDataService.updateBrandPhoto(
            brand.id!, File(compressedFileImage.path));
        print("Image succesfully uploaded!");
        print('\n');
        print(
            '=================================================================================');
  ...
  Future<bool> migrateNotificationsDataMarch17th() async {
    try {
      ...
      for (int i = 0; i < querySnapshotUsers.docs.length; i++) {
        ...
        print('USER WITH ID: ' +
            userId +
            " AND TOTAL NOTIFICATIONS: " +
            querySnapshot.docs.length.toString());
        print('\n');

        for (int i = 0; i < querySnapshot.docs.length; i++) {
          DocumentSnapshot doc = querySnapshot.docs[i];
          if ((doc.data() as Map<String, dynamic>).containsKey('createdAt') ==
              false) {
            NotificationEvent notif = NotificationEvent.fromObjectAllData(
                querySnapshot.docs[i].id, querySnapshot.docs[i]);
            // Get CupertinoSelect
  ...
  Future<bool> migrateEventsDataMarch24th() async {
    try {
      ...
      for (int i = 0; i < querySnapshotEvents.docs.length; i++) {
        Event event = Event.fromObjectAllData(
            querySnapshotEvents.docs[i].id, querySnapshotEvents.docs[i]);
        print(
            '=================================================================================');
        print(
            '=================================================================================');
        ...
        print("Timestamp is $eventTimeStamp");
        // Save TimeStamp Firebase
        await _firestore.collection(eventsCollection).doc(event.id!).update({
      ...
      for (int i = 0; i < querySnapshotUsers.docs.length; i++) {
        ...
        QuerySnapshot querySnapshotUsersEvents = await _firestore
        ...
        for (int i = 0; i < querySnapshotUsersEvents.docs.length; i++) {
          Event event = Event.fromObjectAllData(
              querySnapshotUsersEvents.docs[i].id,
              querySnapshotUsersEvents.docs[i]);
          // Get CupertinoSelect
          DateTime eventDate = DateTime(
            int.parse(event.year!),
            int.parse(event.month!),
      ...
      QuerySnapshot querySnapshotBrands =
          await _firestore.collection(brandsCollection).get();
      for (int i = 0; i < querySnapshotBrands.docs.length; i++) {
        ...
        QuerySnapshot querySnapshotBrandEvents = await _firestore
        ...
        for (int i = 0; i < querySnapshotBrandEvents.docs.length; i++) {
          Event event = Event.fromObjectAllData(
              querySnapshotBrandEvents.docs[i].id,
              querySnapshotBrandEvents.docs[i]);
          // Get CupertinoSelect
          DateTime eventDate = DateTime(
            int.parse(event.year!),
            int.parse(event.month!),
      ...
      QuerySnapshot querySnapshotLocations =
          await _firestore.collection(locationCollection).get();
      for (int i = 0; i < querySnapshotLocations.docs.length; i++) {
        ...
        QuerySnapshot querySnapshotLocationEvents = await _firestore
        ...
        for (int i = 0; i < querySnapshotLocationEvents.docs.length; i++) {
          Event event = Event.fromObjectAllData(
              querySnapshotLocationEvents.docs[i].id,
              querySnapshotLocationEvents.docs[i]);
          // Get CupertinoSelect
          DateTime eventDate = DateTime(
            int.parse(event.year!),
            int.parse(event.month!),
  ...
  Future<bool> getStatistics() async {
    try {
      ...
      print('femaleUsers: $femaleUsers');
      print('maleUsers: $maleUsers');
      ...
      print('Users Clients: ${querySnapshot.docs.length}');
  ...
  Future<bool> migrateEventDataSeptember9th() async {
    try {
      ...
      QuerySnapshot querySnapshot = await _firestore
          .collection(events)
          .where("month", isEqualTo: 10.toString())
          .get();
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        Event event = Event.fromObjectAllData(
            querySnapshot.docs[i].id, querySnapshot.docs[i]);
        ...
        try {
          ...
          for (var i = 0; i < querySnapshotUsers.size; i++) {
            Usuario user = Usuario.fromObjectOnlyCoverData(
                querySnapshotUsers.docs[i].id, querySnapshotUsers.docs[i]);
            await _firestore
                .collection(users)
                .doc(user.id!)
                .collection("Events")
          ...
          print(
              'No document to update: projects/mamba-style/databases/(default)/documents/Users/{userId}/Events/[eventId}');
          usersErrorCnt += 1;
        }
        ...
        try {
          ...
          print('-----------------------------\n');
          QuerySnapshot querySnapshotBrands = await _firestore
              .collection(events)
              .doc(event.id!)
              .collection("Brands")
              .get();
          for (var i = 0; i < querySnapshotBrands.size; i++) {
            Brand brand = Brand.fromObjectOnlyCoverData(
                querySnapshotBrands.docs[i].id, querySnapshotBrands.docs[i]);
            await _firestore
                .collection(brands)
                .doc(brand.id!)
                .collection("Events")
          ...
          print(
              'No document to update: projects/mamba-style/databases/(default)/documents/Brands/{brandId}/Events/[eventId}');
          brandErrorCnt += 1;
        }
        ...
        try {
          ...
          for (var i = 0; i < querySnapshotLocations.size; i++) {
            Location location = Location.fromObjectOnlyCoverData(
                querySnapshotLocations.docs[i].id,
                querySnapshotLocations.docs[i]);
            await _firestore
                .collection(locations)
                .doc(location.id!)
                .collection("Events")
          ...
          print(
              'No document to update: projects/mamba-style/databases/(default)/documents/Locations/{locationId}/Events/[eventId}');
          locationErrorCnt += 1;
        }
    ...
    try {
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        for (int i = 0; i < querySnapshot2.docs.length; i++) {
          Usuario usuario = Usuario.fromObjectOnlyCoverData(
        ...
        for (var i = 0; i < querySnapshotUsers.docs.length; i++) {
          ...
          DocumentSnapshot<Map<String, dynamic>> documentSnapshot2 =
              await _firestore
                  ...
                  .doc(brandId)
                  .get();
          String dateJoined = documentSnapshot2.get("dateJoined");
          // Get Last Event of User
          QuerySnapshot querySnapshotUserEvents = await _firestore
              .collection(users)
              .doc(userId)
              .collection("Events")
              .get();
          ...
          for (int i = 0; i < querySnapshotUserEvents.docs.length; i++) {
            events.add(Event.fromObjectOnlyCoverData(
                querySnapshotUserEvents.docs[i].id,
                querySnapshotUserEvents.docs[i]));
          }
          events.sort((a, b) {
          ...
          if (events.isNotEmpty) {
            ...
            // Updates
            print("lastEventAt: ${lastEvent.doneAt}");
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        if (querySnapshot2.docs.isNotEmpty) {
          ...
          QuerySnapshot querySnapshot3 = await _firestore
              ...
              .doc(userId)
              .collection("Notifications")
              .where("type", isEqualTo: "UserBuysBono_Trainer")
              .get();
          for (int i = 0; i < querySnapshot3.docs.length; i++) {
            NotificationEvent notif = NotificationEvent.fromObjectAllData(
                querySnapshot3.docs[i].id, querySnapshot3.docs[i]);
  ...
  Future<bool> migratePurchaseDataDecember16th() async {
    try {
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        Purchase purchase =
            Purchase.fromObjectAllData(purchaseId, querySnapshot.docs[i]);
        print(
            '=================================================================================');
        print(
            '=================================================================================');
        ...
        // For each Event
        for (int i = 0; i < querySnapshot2.docs.length; i++) {
          Event event = Event.fromObjectOnlyCoverData(
              querySnapshot2.docs[i].id, querySnapshot2.docs[i]);
          // Get the Image Url
  ...
  Future<bool> JMFmigrateUserDataJenuary31() async {
    try {
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        String userId = querySnapshot.docs[i].id;
        print(userId);
        user = Usuario.fromObjectAllData(
            querySnapshot.docs[i].id, querySnapshot.docs[i]);
        print(
            '=================================================================================');
        print(
        ...
        if (querySnapshot2.docs.isNotEmpty) {
          ...
          // Filter type of notification UserBuysBono_Trainer
          if (user.isTrainer!) {
            querySnapshot3 = await _firestore
                .collection(users)
                .doc(userId)
                .collection("Notifications")
                .where("type", isEqualTo: "UserSendBonoRequest_Trainer")
                .get();
            querySnapshot4 = await _firestore
                .collection(users)
                .doc(userId)
                .collection("Notifications")
                .where("type", isEqualTo: "UserCancelBonoRequest_Trainer")
                .get();
          } else {
            querySnapshot3 = await _firestore
                .collection(users)
                .doc(userId)
                .collection("Notifications")
                .where("type", isEqualTo: "UserSendBonoRequest_User")
                .get();
            querySnapshot4 = await _firestore
                .collection(users)
                .doc(userId)
                .collection("Notifications")
                .where("type", isEqualTo: "UserCancelBonoRequest_User")
                .get();
          }
          for (int i = 0; i < querySnapshot3.docs.length; i++) {
            NotificationEvent notif = NotificationEvent.fromObjectAllData(
          ...
          for (int i = 0; i < querySnapshot4.docs.length; i++) {
            NotificationEvent notif = NotificationEvent.fromObjectAllData(
                querySnapshot4.docs[i].id, querySnapshot4.docs[i]);
            print('NOTIF WITH ID: ${notif.id!}');
            // Add Brand Id
            var parameters = [
              notif.parameters[0],
      ...
      for (int i = 0; i < querySnapshotBrands.docs.length; i++) {
        ...
        if (querySnapshot2.size > 0) {
          baseImage = ImageObject.fromObjectAllData(
                  querySnapshot2.docs[0].id, querySnapshot2.docs[0])
              .url!;
          await _firestore
              .collection(brands)
              .doc(brandId)
              .collection("Images")
              .doc(querySnapshot2.docs[0].id)
              .update({
            "isBaseImage": true,
          });
        } else {
        ...
        QuerySnapshot querySnapshotLocations = await _firestore
            ...
            .doc(brandId)
            .collection("Locations")
            .where("isBaseLocation", isEqualTo: true)
            .get();
        print('3rd cause');
        DocumentSnapshot<Map<String, dynamic>> documentSnapshot =
            await _firestore
                .collection(locations)
                .doc(querySnapshotLocations.docs[0].id)
                .get();
  ...
  Future<bool> JMFassignBrandIdToUserEvents() async {
    try {
      ...
      for (int i = 0; i < querySnapshotUsers.docs.length; i++) {
        String userId = querySnapshotUsers.docs[i].id;
        print(userId);
        QuerySnapshot querySnapshotEventsUser = await _firestore
            .collection(users)
            .doc(userId)
            .collection("Events")
            .get();
        ...
        for (int j = 0; j < querySnapshotEventsUser.docs.length; j++) {
          ...
          print(eventId);
          DocumentSnapshot<Map<String, dynamic>> documentSnapshotEvent =
              await _firestore
                  .collection(events)
                  .doc(querySnapshotEventsUser.docs[j].id)
                  .get();
          Event event = Event.fromObjectAllData(
              documentSnapshotEvent.id, documentSnapshotEvent);
          await _firestore
              .collection(users)
              .doc(userId)
              .collection("Events")
  ...
  Future<bool> JMFassignBrandIdToLocationEvents() async {
    try {
      ...
      for (int i = 0; i < querySnapshotLocations.docs.length; i++) {
        String locationId = querySnapshotLocations.docs[i].id;
        QuerySnapshot querySnapshotEventsLocation = await _firestore
            .collection(locations)
            .doc(locationId)
            .collection("Events")
            .get();
        ...
        for (int j = 0; j < querySnapshotEventsLocation.docs.length; j++) {
          ...
          DocumentSnapshot<Map<String, dynamic>> documentSnapshotEvent =
              await _firestore
                  .collection(events)
                  .doc(querySnapshotEventsLocation.docs[j].id)
                  .get();
          Event event = Event.fromObjectAllData(
              documentSnapshotEvent.id, documentSnapshotEvent);
          await _firestore
  ...
  Future<bool> JMFassignGeoPointToLocations() async {
    try {
      ...
      for (int i = 0; i < querySnapshotLocations.docs.length; i++) {
        String locationId = querySnapshotLocations.docs[i].id;
        print(locationId);
        Location location = Location.fromObjectAllData(
            locationId, querySnapshotLocations.docs[i]);
      ...
      for (int i = 0; i < querySnapshotBrands.docs.length; i++) {
        ...
        if (brand.endDatePay != null && brand.subscriptionId != null) {
          ...
          DocumentSnapshot documentSnapshot = await _firestore
              ...
              .get();
          Subscription sub = Subscription.fromObjectAllData(
  ...
  Future<bool> JBmigratePurchaseDataJune16th() async {
    try {
      ...
      QuerySnapshot querySnapshotPurchases = await _firestore
      ...
      for (int i = 0; i < querySnapshotPurchases.size; i++) {
        ...
        Purchase purchase = Purchase.fromObjectAllData(
            purchaseId, querySnapshotPurchases.docs[i]);
        print(
            '=================================================================================');
        print(
            '=================================================================================');
        print('PURCHASE WITH ID: ' +
            purchase.id! +
            " AND USER: " +
            purchase.userId!);
        print('\n');
        ...
        await _firestore.collection(purchasesCollection).doc(purchase.id!).set({
        ...
        if (querySnapshotEvents.size > 0) {
          ...
          for (int i = 0; i < querySnapshotEvents.size; i++) {
            String eventId = querySnapshotEvents.docs[i].id;
            Event event = Event.fromObjectOnlyCoverData(
                eventId, querySnapshotEvents.docs[i]);
  ...
  Future<bool> JBupdatePurchaseDataJune16th() async {
    try {
      ...
      String brandsCollection = "7777 Brands";
      String purchasesCollection = "7777 Purchases";
      QuerySnapshot querySnapshotPurchases =
          await _firestore.collection(purchasesCollection).get();
      for (int i = 0; i < querySnapshotPurchases.size; i++) {
        String purchaseId = querySnapshotPurchases.docs[i].id;
        Purchase purchase = Purchase.fromObjectAllData(
            purchaseId, querySnapshotPurchases.docs[i]);
  ...
  Future<bool> JBparcheUserJoinsEventNovembre23th() async {
    int i = 0;
    try {
      print('\n');
      print('-----------------------------');
      print('DATA MIGRATION 23RD NOVEMBER FOR BUG 2023');
      print('-----------------------------');
      print('\n');

      // PROBLEMA: Desde el dia 22/11 18:00 fins al dia 23/11 12:30, la cloudfunction userJoinsEvent no ha funcionat correctament perque hi habia un bug
      // quan vaig afegir lo del freeSession. Això ha creat que nos creesin els events a User/Events i tampoc s'actualitzessin el nombre de Clients per sessió.

      // SOLUCIÓ:
      // Filtrar tots els Events del dia 22-11 fins al 22-12 de Events
      // Mirar tots els usuaris que estan registrats /Event/Users
      // Comprobar que Users/Events exiteix i sino crearlo.
      // Actualitzar numClient i numTrainers a /Events/ - Brand/Events - User/Events - Location/Events (tenir en compte també el Private Event)
      // Després asegurar que has actualitzat el lastActiveAt.

      DateTime startDate = DateTime(2023, 11, 22);
      DateTime endDate = DateTime(2023, 12, 22);
      ...
      String usersCollection = "Users";
      String brandsCollection = "Brands";
      String locationsCollection = "Locations";

      QuerySnapshot querySnapshotEvents = await _firestore
          .collection(eventsCollection)
          .where("doneAt", isGreaterThanOrEqualTo: start)
          .where("doneAt", isLessThanOrEqualTo: end)
          .get();
      print("Events de $startDate fins a $endDate");
      print(querySnapshotEvents.docs.length);

      // Filtrar tots els Events del dia 22-11 fins al 22-12 de Events
      for (DocumentSnapshot doc in querySnapshotEvents.docs) {
        print(
            '=================================================================================');
        // Event
        Event evt = Event.fromObjectAllData(doc.id, doc);
        print("Event ID: ${evt.id}");
        // Mirar tots els usuaris que estan registrats /Event/Users
        QuerySnapshot querySnapshotEventUsers = await _firestore
            .collection(eventsCollection)
            .doc(evt.id)
            .collection("Users")
            .get();
        ...
        int numClients = 0;
        int numTrainers = 0;
        for (DocumentSnapshot docUser in querySnapshotEventUsers.docs) {
          Usuario user = Usuario.fromObjectAllData(docUser.id, docUser);
          if (user.isTrainer!) {
            numTrainers += 1;
          } else {
            numClients += 1;
          ...
          DocumentSnapshot document = await _firestore
              ...
              .get();
          if (document.exists) {
            // Document has been successfully found and created via Cloud Function
          } else {
            print("Cloud Function Problem");
            // Document NOT created via Cloud Function
            // Calculate Free Session
            bool freeSession = false;
            if ((docUser.data() as Map<String, dynamic>)
                .containsKey('freeSession')) {
              freeSession = docUser.get("freeSession");
            }
            // Add Event To Users Event Subcollection
            print("Add Event To Users Event Subcollection");
            await _firestore
                .collection(usersCollection)
                .doc(user.id)
                .collection("Events")
            ...
            if (evt.isPrivate == true) {
              print("Event is Private, adding here too...");
              await _firestore
                  .collection(usersCollection)
                  .doc(user.id)
                  .collection("Events")
            ...
            await _firestore
                ...
                .doc(user.id)
                .update({
              "lastEventAt": evt.doneAt,
            });
          }
        }
        print("Actualitzar numClient i numTrainers a:");
        // Actualitzar numClient i numTrainers a /Events/
        await _firestore.collection(eventsCollection).doc(evt.id).update({
          "numClients": numClients,
          "numTrainers": numTrainers,
        });
        print("Events");
        // Actualitzar numClient i numTrainers a User/Events
        for (DocumentSnapshot docUser in querySnapshotEventUsers.docs) {
          Usuario user = Usuario.fromObjectAllData(docUser.id, docUser);
          await _firestore
              .collection(usersCollection)
        ...
        print("Users/Events");
        // Actualitzar numClient i numTrainers a Brand/Events
        await _firestore
            .collection(brandsCollection)
            .doc(evt.brandID)
            .collection("Events")
